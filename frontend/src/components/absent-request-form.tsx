...
import { z } from "zod";
import { createAbsentRequest, type AbsentRequest } from "../lib/api";
...
export function AbsentRequestForm({
  employeeName,
  onCreated,
}: {
  employeeName: string;
  onCreated: (request: AbsentRequest) => void;
}) {
  const form = useForm({
    defaultValues: {
    ...
    >
      ...
      <div className="form-grid">
        ...
        <form.Field name="absent_type">
          {(field) => (
            <div>
              <FieldLabel>Loại vắng mặt</FieldLabel>
              <select
                className="input"
                value={field.state.value}
                onChange={(event) => field.handleChange(event.target.value)}
        ...
        <form.Field
          name="start_date"
          validators={{
            onChange: ({ value }) => (!value ? "Chọn ngày bắt đầu" : undefined),
          }}
        >
          {(field) => (
            <div>
        ...
        <form.Field
          ...
          validators={{
            onChange: ({ value }) =>
              !value ? "Chọn ngày kết thúc" : undefined,
          }}
        >
          {(field) => (
            <div>
      ...
      <form.Field
        ...
        validators={{
          onChange: ({ value }) =>
            value.length < 5 ? "Lý do cần ít nhất 5 ký tự" : undefined,
        }}
      >
        {(field) => (
          <div>
            ...
            <Textarea
              ...
              onBlur={field.handleBlur}
              onChange={(event) => field.handleChange(event.target.value)}
              placeholder="Mô tả ngắn gọn lý do vắng mặt..."
            />
            {errorText(field.state.meta.errors) && (
              <span className="error">
                {errorText(field.state.meta.errors)}
