...
import { z } from "zod";
import { createLeaveRequest, type LeaveRequestPayload } from "../lib/api";
...
const schema = z.object({
  ...
  leave_type: z.string().min(2, "Chọn loại nghỉ phép"),
...
function errorText(errors: unknown[]) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      const result = await createLeaveRequest(value);
      onCreated(result);
      form.reset();
    },
  });
  ...
  return (
    <form className="leave-form" onSubmit={(event) => { event.preventDefault(); event.stopPropagation(); void form.handleSubmit(); }}>
      <div className="form-heading">
        <span className="eyebrow">New request</span>
        <h2>Gửi yêu cầu nghỉ phép</h2>
      ...
      <div className="form-grid">
        ...
        <form.Field name="leave_type">
          {(field) => <div><FieldLabel>Loại nghỉ</FieldLabel><select className="input" value={field.state.value} onChange={(event) => field.handleChange(event.target.value)}><option>Annual leave</option><option>Sick leave</option><option>Personal leave</option></select></div>}
        </form.Field>
      ...
      <form.Field name="reason" validators={{ onChange: ({ value }) => value.length < 5 ? "Lý do cần ít nhất 5 ký tự" : undefined }}>
        {(field) => <div><FieldLabel>Lý do</FieldLabel><Textarea rows={4} value={field.state.value} onBlur={field.handleBlur} onChange={(event) => field.handleChange(event.target.value)} placeholder="Mô tả ngắn gọn lý do nghỉ..." />{errorText(field.state.meta.errors) && <span className="error">{errorText(field.state.meta.errors)}</span>}</div>}
      </form.Field>
